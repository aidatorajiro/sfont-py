...
vector<string> string_split(string str, char del) {
    int first = 0;
...
void SoundFont::readSignature(const char* signature)
      ...
      if (memcmp(fourcc, signature, 4) != 0)
            throw(string("fourcc <%1> expected").arg(signature));
      }
...
void SoundFont::readSection(const char* fourcc, int len)
      ...
      switch(FOURCC(fourcc[0], fourcc[1], fourcc[2], fourcc[3])) {
            ...
            default:
                  skip(len);
                  throw(string("unknown fourcc <%1>").arg(fourcc));
            }
      }
...
void SoundFont::readBag(int len, vector<Zone*>* zones)
      {
      if (len % 4)
...
void SoundFont::readMod(int size, vector<Zone*>* zones)
      {
      foreach(Zone* zone, *zones) {
...
void SoundFont::readGen(int size, vector<Zone*>* zones)
      {
      if (size % 4)
      ...
      if (size != 4)
            throw(string("generator list size mismatch %1 != 4").arg(size));
      skip(size);
      }
...
bool SoundFont::compress(fstream f, double oggQuality, double oggAmp, qint64 oggSerial)
      {
      file = f;
      _compress = true;
...
bool SoundFont::uncompress(fstream f)
      {
      file = f;
      _compress = false;
      return write();
...
void SoundFont::writeBag(const char* fourcc, vector<Zone*>* zones)
      {
      write(fourcc, 4);
      int n = zones->size();
      writeDword((n + 1) * 4);
...
void SoundFont::writeMod(const char* fourcc, const vector<Zone*>* zones)
      {
      write(fourcc, 4);
      int n = 0;
      foreach(const Zone* z, *zones)
...
void SoundFont::writeGen(const char* fourcc, vector<Zone*>* zones)
      {
      write(fourcc, 4);
      int n = 0;
      foreach(const Zone* z, *zones)
...
int SoundFont::writeCompressedSample(Sample* s)
      ...
      vorbis_comment_add(&vc, string("AMP=%1\0").arg(_oggAmp).toStdString().c_str());
...
static bool checkInstrument(vector<int> pnums, vector<Preset*> presets, int instrIdx)
      {
...
static bool checkInstrument(vector<Preset*> presets, int instrIdx) {
...
static bool checkSample(vector<int> pnums, vector<Preset*> presets, vector<Instrument*> instruments,
   int sampleIdx)
      {
      int idx = 0;
...
static bool checkSample(vector<Preset*> presets, vector<Instrument*> instruments,
   int sampleIdx)
      {
      int idx = 0;
      foreach(Instrument* instrument, instruments) {
...
bool SoundFont::writeCode(vector<int> pnums)
      {
      printf("write code\n");
...
bool SoundFont::writeXml(fstream f)
      {
      Xml xml(f);

      xml.header();
      xml.stag("Sfont");
      xml.tag("version", string("%1.%2").arg(version.major).arg(version.minor));
      if (name)
      ...
      foreach(Preset* p, presets) {
            xml.stag(string("Preset name=\"%1\" preset=\"%2\" bank=\"%3\"")
      ...
      foreach(Instrument* instrument, instruments) {
            xml.stag(string("Instrument name=\"%1\"").arg(instrument->name));
            foreach(Zone* z, instrument->zones)
                  write(xml, z);
            xml.etag();
            }
      int idx = 0;
      foreach(Sample* s, samples) {
            xml.stag(string("Sample name=\"%1\"").arg(s->name));
            ...
            xml.etag();
            writeSampleFile(s, string("%1").arg(idx));
            ++idx;
            }
      xml.etag();
      return true;
...
void SoundFont::write(Xml& xml, Zone* z)
      ...
      foreach(GeneratorList* g, z->generators) {
            ...
            if (g->gen == Gen_KeyRange || g->gen == Gen_VelRange)
                  xml.tagE(string("Generator name=\"%1\" lo=\"%2\" hi=\"%3\"")
                  ...
                  xml.tagE(string("Generator name=\"%1\" val=\"%2\"")
...
bool SoundFont::writeSampleFile(Sample* s, string name)
      {
      string path = "waves/" + name + ".ogg";
...
bool SoundFont::readXml(fstream f)
      {
      QDomDocument doc;
      int line, column;
      string err;
...
int SoundFont::writeUncompressedSample(Sample* s)
{
    ...
    if (ov_open_callbacks(&vorbisData, &vf, nullptr, 0, ovCallbacks) == 0)
    {
        ...
        for (int i = 0; i < vf.vc->comments; i++)
        {
            ...
            {
                stringList split = string_split(comment, '=');
                if (split.size() == 2)
                {
                    bool ok = false;
