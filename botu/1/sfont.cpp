...
void SoundFont::readBag(int len, list<Zone*>* zones)
      {
      if (len % 4)
...
void SoundFont::readMod(int size, list<Zone*>* zones)
      {
      foreach(Zone* zone, *zones) {
...
void SoundFont::readGen(int size, list<Zone*>* zones)
      {
      if (size % 4)
...
void SoundFont::writeBag(const char* fourcc, list<Zone*>* zones)
      {
      write(fourcc, 4);
      int n = zones->size();
      writeDword((n + 1) * 4);
...
void SoundFont::writeMod(const char* fourcc, const list<Zone*>* zones)
      {
      write(fourcc, 4);
      int n = 0;
      foreach(const Zone* z, *zones)
...
void SoundFont::writeGen(const char* fourcc, list<Zone*>* zones)
      {
      write(fourcc, 4);
      int n = 0;
      foreach(const Zone* z, *zones)
...
static bool checkInstrument(list<int> pnums, list<Preset*> presets, int instrIdx)
      {
...
static bool checkInstrument(list<Preset*> presets, int instrIdx) {
...
static bool checkSample(list<int> pnums, list<Preset*> presets, list<Instrument*> instruments,
   int sampleIdx)
      {
      int idx = 0;
...
static bool checkSample(list<Preset*> presets, list<Instrument*> instruments,
   int sampleIdx)
      {
      int idx = 0;
      foreach(Instrument* instrument, instruments) {
...
bool SoundFont::writeCode(list<int> pnums)
      {
      printf("write code\n");
...
int SoundFont::writeUncompressedSample(Sample* s)
{
    ...
    if (ov_open_callbacks(&vorbisData, &vf, nullptr, 0, ovCallbacks) == 0)
    {
        ...
        for (int i = 0; i < vf.vc->comments; i++)
        {
            ...
            {
                stringList split = comment.split('=');
                if (split.size() == 2)
                {
                    bool ok = false;
